# 원고지 편집기 (웹)

키보드로 쓴 글을 20×30 원고지 칸에 한 자씩 놓고, 손글씨 글꼴로 인쇄하거나 PDF로 내보내는 웹 앱.
데스크톱(PySide6) 버전과 배치·치수 규칙이 같고, 계산 로직만 JavaScript로 옮겼다.

## 파일

| 파일 | 하는 일 |
|---|---|
| `index.html` | 화면 틀과 도구 모음 |
| `layout.js` | 글자 → (줄, 칸) 배치 계산 |
| `paper.js` | 원고지와 글자를 SVG(단위 mm)로 그리기 |
| `app.js` | 입력·글꼴·저장·인쇄 연결 |
| `sample.js` | 처음 열었을 때 채워 두는 예시 글 (윤동주 「서시」) |
| `fonts.js`, `fonts/` | 글꼴 목록과 woff2 파일 (`tools/build_fonts.py`가 만든다) |
| `fonts/tl-grid.woff2` | 입력칸에 씌우는 격자 글꼴 (`tools/build_grid_font.py`가 만든다) |
| `manifest.webmanifest`, `sw.js`, `icon-*.png` | 홈 화면에 추가해서 앱처럼 쓰기 위한 것 |

## 폰에서 쓰기 — GitHub Pages에 올리기

글꼴이 26MB라 인터넷 연결이 필요하다. 받은 글꼴은 브라우저 캐시에 **최근 3개까지만**
남기고, 먼저 받은 것부터 버린다 (큐). 도구 모음의 점 3개 아이콘이 지금 저장된 글꼴 수를
보여 주고, 누르면 목록이 아래쪽에 나온다.

```bash
cd web
git init -b main
git add .
git commit -m "원고지 편집기 웹 앱"
gh repo create the-letter --public --source=. --push
```

올린 뒤 저장소 **Settings → Pages → Source**를 `main` 브랜치 `/ (root)`로 지정한다
(또는 `gh api -X POST repos/Bbackbagi2/the-letter/pages -f "source[branch]=main" -f "source[path]=/"`).

1~2분 뒤 `https://bbackbagi2.github.io/the-letter/` 가 열린다.
폰 사파리/크롬에서 그 주소를 열고 **공유 → 홈 화면에 추가**를 누르면 앱처럼 실행된다.

> 글꼴 파일을 함께 올리는 것은 재배포에 해당한다. 나눔손글씨(OFL)와 독립운동가 서체(GS칼텍스)의
> 재배포·출처 표기 조건을 확인한 뒤 공개 저장소로 둘지 정하는 게 좋다. 확실하지 않은 부분이다.

## 용지 형식

| 형식 | 내용 |
|---|---|
| A4 원고지 | A4 한 장에 6.6mm 칸. 기준 PDF와 같다 |
| 엽서 3단접기 125×250mm | A4에 인쇄해 125×250mm로 자르고 3단으로 접어 엽서 크기로 만든다 |

3단접기 치수는 `resources/pdf/원고지_20x30_3단접기_12.5x25cm.pdf`에서 그대로 읽어 왔다.
칸 5.45mm(A4의 0.826배), 격자 109×227.701mm를 (50.5, 38.41)에 놓고, 재단 상자는
A4 정중앙의 125×250mm다. 접는 선은 **9번째·20번째 줄 앞 띠의 정중앙**이라 칸을 가르지
않는다 (단별 9·11·10줄). 모서리에는 재단 표시, 양옆에는 접기 눈금, 아래에는 안내 문구가
들어간다. 잘려 나갈 자리는 **화면에서만** 옅게 덮어 두고 인쇄에는 넣지 않는다.

## 맥에서 바로 열어 보기

```bash
cd web && python3 -m http.server 8765
```

`http://127.0.0.1:8765` 로 접속한다. (`file://`로 열면 ES 모듈이 막혀 동작하지 않는다.)

## 글꼴을 바꾸거나 굵기를 고칠 때

`resources/font/`에 글꼴을 넣거나 빼고 다시 변환한다.

```bash
source .venv/bin/activate && python3 tools/build_fonts.py
```

- 변환할 때 글리프 감김 방향을 정리한다 (반대로 감긴 획이 macOS에서 연하게 찍히는 문제).
- 획을 가늘게 할 글꼴은 `tools/build_fonts.py`의 `THINNING` 표에 적는다 (획 한쪽에서 깎아 들어갈 깊이, em 1000 기준).
- 온점·쉼표처럼 작은 글리프는 깎으면 점 자체가 작아지므로 건드리지 않는다 (`SMALL_GLYPH`).
- 온점은 글꼴에서 작게 나와 `paper.js`의 `MARK_SCALES`로 1.3배 크게 그린다.
- 글꼴은 CSS·FontFace에 `tl-font-N` 별명으로 등록한다 (한글 이름이 기기마다 다르게 해석될 여지를 없앤다).
- 글꼴 파일을 다시 만들었으면 `sw.js`의 `FONTS` 캐시 이름 숫자를 올려야 폰에서 새로 받는다.

## 쓰는 법

### 마우스·키보드

| 조작 | 하는 일 |
|---|---|
| 클릭 | 커서 옮기기 (끌면 선택, Shift+클릭·Shift+방향키로 넓히기) |
| ⌘A / ⌘C / ⌘X / ⌘V | 전체 선택·복사·잘라내기·붙여넣기 |
| ⌘Z / ⌘⇧Z | 실행 취소 / 다시 실행 |
| ⌘⌫ | 그 줄 첫 칸까지 지우기 |
| ⌘S | txt로 내려받기 (도구 모음 "이름"에 적은 파일 이름을 쓴다) |
| 세로쓰기 | 종이를 돌려 세로로 쓰기 (인쇄는 A4 세로 그대로, 글자만 눕는다) |
| 인쇄 · PDF | 브라우저 인쇄 창 — 배율 100%, 여백 없음으로 두어야 칸이 제 크기로 나온다.
  PDF 파일 이름은 "이름"에 적은 것을 쓴다 (인쇄 직전에 문서 제목을 바꿔 둔다) |
| 용지 | 인쇄 형식 선택 (아래) |

### 폰·태블릿 (손가락)

글이 담기는 곳은 종이 위에 덮인 **투명한 입력칸**이다. 글자 폭이 모두 한 칸인 격자 글꼴
(`tl-grid`)을 씌워 두었기 때문에 브라우저가 글자를 원고지 칸과 같은 격자에 배치한다.
그래서 조작은 기기가 원래 하던 방식 그대로다.

| 조작 | 하는 일 |
|---|---|
| 두드리기 | 커서 옮기기 + 키보드 올라옴 |
| 두 번 / 세 번 두드리기 | 낱말 / 줄 선택 (기기 기본) |
| 길게 누르기 | 돋보기와 선택 핸들, 복사·붙여넣기 풍선 (기기 기본) |
| 끌기 · 두 손가락 | 화면 밀기 · 확대 (기기 기본) |
| 받아쓰기·글자 인식 | 키보드의 기능을 그대로 쓸 수 있다 |

따로 만든 편집 버튼은 없앴다. 기기가 제공하는 편집 UI가 칸에 맞게 뜨기 때문이다.

글은 브라우저에 자동 저장되어 다시 열면 이어서 쓸 수 있고, 커서는 글 끝에 놓인다.
